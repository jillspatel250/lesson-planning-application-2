...
export default function PracticalPlanningForm({
    ...
    const loadPsoPeoData = async () => {
      if (lessonPlan.subject?.id) {
        ...
        try {
          ...
          // Check if subject has PSO/PEO data
          if (
            subjectData?.pso?.items &&
            Array.isArray(subjectData.pso.items) &&
            subjectData.pso.items.length > 0
          ) {
            psoData = subjectData.pso.items;
          }

          if (
            subjectData?.peo?.items &&
            Array.isArray(subjectData.peo.items) &&
            subjectData.peo.items.length > 0
          ) {
            peoData = subjectData.peo.items;
          }
          ...
          // If no PSO/PEO data found in subject, try to get from department
          if (
            (psoData.length === 0 || peoData.length === 0) &&
            subjectData?.department_id
          ) {
            console.log(
              "Fetching department PSO/PEO data for department:",
              subjectData.department_id
            );
            ...
            if (!deptError && deptData) {
              if (
                psoData.length === 0 &&
                deptData.pso_data &&
                Array.isArray(deptData.pso_data)
              ) {
                psoData = deptData.pso_data;
                console.log("Using department PSO data:", psoData);
              }
              ...
              if (
                peoData.length === 0 &&
                deptData.peo_data &&
                Array.isArray(deptData.peo_data)
              ) {
                peoData = deptData.peo_data;
                console.log("Using department PEO data:", peoData);
              }
          ...
          if (
            ...
            const { data: departmentSubjects, error: deptSubjectsError } =
              await supabase
                .from("subjects")
                .select("pso, peo")
                .eq("department_id", subjectData.department_id)
  ...
  useEffect(() => {
    const fetchTermDates = async () => {
      try {
        ...
        console.log(
          "DEBUG - Fetching term dates for subject code:",
          lessonPlan.subject.code
        );
  ...
  useEffect(() => {
    ...
    const autoLoadDraft = async () => {
      ...
      console.log(
        "✅ PRACTICAL AUTO-LOAD: Required data available - Faculty ID:",
        facultyId,
        "Subject ID:",
        subjectId
      );
      ...
      if (hasExistingData) {
        console.log(
          "⏭️ PRACTICAL AUTO-LOAD: Practicals already loaded, skipping auto-load"
        );
        setIsLoadingDraft(false);
        return;
      }
        ...
        if (result.success && result.data) {
          ...
          if (
            ...
            const validPracticals = data.practicals.map(
              (practical: any, index: number) => {
                const validPractical = {
                  ...
                  id: practical.id || `practical${index + 1}`,
                  practical_aim: practical.practical_aim || "",
                  associated_units: Array.isArray(practical.associated_units)
                ...
                console.log(
                  `🔍 PRACTICAL AUTO-LOAD: Processed practical ${index + 1}:`,
                  validPractical
                );
                return validPractical;
              }
            ...
            if (data.savedAt) {
              setLastSaved(new Date(data.savedAt));
              console.log(
                "🔍 PRACTICAL AUTO-LOAD: Set last saved time:",
                data.savedAt
              );
            } else {
              setLastSaved(new Date());
              console.log(
                "🔍 PRACTICAL AUTO-LOAD: Set current time as last saved"
              );
            }
          } else {
            console.log(
              "❌ PRACTICAL AUTO-LOAD: No valid practical data found in draft"
            );
            console.log("🔍 PRACTICAL AUTO-LOAD: Data structure:", {
              hasPracticals: !!data.practicals,
              isArray: Array.isArray(data.practicals),
          ...
          console.log(
            "❌ PRACTICAL AUTO-LOAD: No draft found or failed to load"
          );
          console.log("🔍 PRACTICAL AUTO-LOAD: Result details:", {
            success: result.success,
            hasData: !!result.data,
    ...
    if (facultyId && subjectId) {
      console.log(
        "✅ PRACTICAL AUTO-LOAD: Conditions met, calling autoLoadDraft"
      );
      autoLoadDraft();
    } else {
      console.log(
        "❌ PRACTICAL AUTO-LOAD: Conditions not met, skipping auto-load"
      );
      console.log("🔍 PRACTICAL AUTO-LOAD: Missing:", {
        facultyId: !!facultyId,
        subjectId: !!subjectId,
  ...
  const generateWeekOptions = (startDateStr: string, endDateStr: string) => {
    ...
    try {
      ...
      return Array.from({ length: numWeeks }, (_, i) => {
        ...
        // If the end date exceeds the term end date, use the term end date
        if (weekEndDate > endDate) {
          return `Week ${i + 1} (${formatDate(weekStartDate)} - ${formatDate(
            endDate
          )})`;
        }
    ...
    <div className="p-6">
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          />
          {practicalTasksError && (
            <p className="text-red-500 text-xs mt-1">{practicalTasksError}</p>
          )}
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                checked={
                  showOtherEvaluationInput ||
                  (currentPractical.evaluation_methods || []).some((m) =>
                    m.startsWith("Other:")
                  )
                }
                onCheckedChange={(checked) => {
                ...
                onClick={() => {
                  if (otherEvaluationValue.trim()) {
                    ...
                    handlePracticalChange(
                      activePractical,
                      "evaluation_methods",
                      [...current, `Other: ${otherEvaluationValue.trim()}`]
                    );
                    setOtherEvaluationValue("");
                  }
                }}
          ...
          {(currentPractical.evaluation_methods || []).length > 0 && (
            <div className="mt-2">
              <Label className="text-sm text-gray-500">Selected Methods:</Label>
              <div className="flex flex-wrap gap-2 mt-1">
                {(currentPractical.evaluation_methods || []).map(
                    ...
                    >
                      ...
                      <button
                        onClick={() => {
                          const updated = (
                            currentPractical.evaluation_methods || []
                          ).filter((m: string, i: number) => i !== idx);
                          handlePracticalChange(
                            activePractical,
                            "evaluation_methods",
                            updated
        ...
        <div>
          ...
          />
          {referenceError && (
            <p className="text-red-500 text-xs mt-1">{referenceError}</p>
          )}
        </div>
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(value) => {
                const current = currentPractical.co_mapping || [];
                if (!current.includes(value)) {
                  handlePracticalChange(activePractical, "co_mapping", [
                    ...current,
                    value,
                  ]);
                }
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(value) => {
                const current = currentPractical.pso_mapping || [];
                if (!current.includes(value)) {
                  handlePracticalChange(activePractical, "pso_mapping", [
                    ...current,
                    value,
                  ]);
                }
                ...
                return (
                  <Badge key={psoId} variant="secondary" className="text-xs">
                    ...
                    <button
                      onClick={() => {
                        const updated = (
                          currentPractical.pso_mapping || []
                        ).filter((id: string) => id !== psoId);
                        handlePracticalChange(
                          activePractical,
                          "pso_mapping",
                          updated
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(value) => {
                const current = currentPractical.peo_mapping || [];
                if (!current.includes(value)) {
                  handlePracticalChange(activePractical, "peo_mapping", [
                    ...current,
                    value,
                  ]);
                }
                ...
                return (
                  <Badge key={peoId} variant="secondary" className="text-xs">
                    ...
                    <button
                      onClick={() => {
                        const updated = (
                          currentPractical.peo_mapping || []
                        ).filter((id: string) => id !== peoId);
                        handlePracticalChange(
                          activePractical,
                          "peo_mapping",
                          updated
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {bloomsTaxonomyOptions.map((taxonomy) => (
              <div key={taxonomy} className="flex items-center space-x-2">
                <Checkbox
                  id={`taxonomy-${taxonomy}`}
                  checked={(currentPractical.blooms_taxonomy || []).includes(
                    taxonomy
                  )}
                  onCheckedChange={(checked) => {
                    const current = currentPractical.blooms_taxonomy || [];
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {skillMappingOptions.map((skill) => (
              <div key={skill} className="flex items-center space-x-2">
                <Checkbox
                  id={`skill-${skill}`}
                  checked={(currentPractical.skill_mapping || []).includes(
                    skill
                  )}
                  onCheckedChange={(checked) => {
                    const current = currentPractical.skill_mapping || [];
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                checked={
                  showOtherSkillInput ||
                  (currentPractical.skill_mapping || []).some((s) =>
                    s.startsWith("Other:")
                  )
                }
                onCheckedChange={(checked) => {
        ...
        <div>
          ...
          <Textarea
            ...
            rows={3}
          />
          {skillObjectivesError && (
            <p className="text-red-500 text-xs mt-1">{skillObjectivesError}</p>
          )}
        </div>
